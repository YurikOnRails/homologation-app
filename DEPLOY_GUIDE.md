...
```bash
# 1. Заполнить .env (контакты + Stripe Payment Link)
cp .env.example .env && nano .env

# 2. Добавить credentials (Stripe, Google, SMTP)
EDITOR="nano" bin/rails credentials:edit

# 3. Обновить config/deploy.yml
#    - servers.web → реальный IP сервера
#    - proxy.ssl: true + proxy.host → домен
#    - env.clear.APP_HOST → домен

# 4. Добавить RAILS_MASTER_KEY в .kamal/secrets
echo "RAILS_MASTER_KEY=$(cat config/master.key)" > .kamal/secrets

# 5. Первый деплой
kamal setup

# 6. Создать super_admin
kamal console
...
### Rails credentials — полная структура
...
smtp:
  user_name: ...
  password: ...
  address: smtp.gmail.com   # или ваш провайдер
  port: 587
