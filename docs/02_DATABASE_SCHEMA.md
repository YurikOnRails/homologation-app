# Database Schema

## How to View

1. Open [dbdiagram.io](https://dbdiagram.io)
2. Copy contents of `docs/02_DATABASE_SCHEMA.dbml`
3. Paste into the editor

---

## Tables Overview

| Group | Table | Color | Purpose |
|---|---|---|---|
| Auth | `users` | 🔵 | All users + guardian fields for minors |
| Auth | `roles` | 🔵 | 4 roles: super_admin, coordinator, teacher, student |
| Auth | `user_roles` | 🔵 | Join table (user can have multiple roles) |
| Auth | `sessions` | 🔵 | Rails 8 auth sessions |
| Teachers | `teacher_profiles` | 🟠 | Level, hourly rate, permanent meeting link |
| Teachers | `teacher_students` | 🟠 | Many-to-many, assigned by coordinator |
| Teachers | `lessons` | 🟠 | Calendar: scheduled lessons with meeting links |
| Business | `homologation_requests` | 🔴 | Student requests for document homologation |
| Chat | `conversations` | 🟢 | Request chats + teacher-student chats (ordered by `last_message_at`) |
| Chat | `messages` | 🟢 | Chat messages (real-time via Action Cable) |
| Chat | `conversation_participants` | 🟢 | Tracks who is in each conversation + last read time |
| Notifications | `notifications` | 🟡 | In-app notifications |
| CRM | `amo_crm_tokens` | 🟣 | AmoCRM OAuth2 tokens |

Total: **13 tables** (+ Active Storage auto-generated by Rails)

## Chat: Conversations & Participants

`conversations` has a `last_message_at` column (updated on every new message) used for efficient ordering in the inbox — no subquery needed.

`conversation_participants` is a join table between `users` and `conversations`. Each row records the participant and a `last_read_at` timestamp, which drives unread-message counts displayed in the sidebar and notification badge.

| Column on `conversation_participants` | Purpose |
|---|---|
| `conversation_id` | FK → conversations |
| `user_id` | FK → users |
| `last_read_at` | Timestamp of the last message the user has seen |

Unread count: `messages.where("created_at > ?", participant.last_read_at).count`.

## Minor / Guardian (Variant 1.5)

Students can be minors (parents pay) or adults (pay themselves).

| Field on `users` | Purpose |
|---|---|
| `is_minor` | `true` if under 18 |
| `guardian_name` | Parent name |
| `guardian_email` | Parent email (Stripe invoices go here) |
| `guardian_phone` | Parent phone |
| `guardian_whatsapp` | Parent WhatsApp (AmoCRM uses this if minor) |
| `guardian_user_id` | Optional FK → users (future: parent login) |

**MVP:** guardian fields are text, parent gets invoices/notifications by email.
**Future:** parent registers, links to child via `guardian_user_id`, can log in and see status.

## Notification Preferences & Telegram

| Field on `users` | Purpose |
|---|---|
| `telegram_chat_id` | Telegram Bot chat ID, set when user clicks /start in bot |
| `notification_telegram` | `true` = send notifications to Telegram (default: false) |
| `notification_email` | `true` = send notifications to email (default: true) |

**Flow:** User clicks "Connect Telegram" in profile → opens `t.me/YourBot` → clicks /start → bot receives `chat_id` → saved to user → `notification_telegram` set to `true`.

**Channels:** In-app (always) + Email (default on) + Telegram (opt-in via button). User can toggle each in profile settings.

## Soft Delete & Data Retention

Tables `users` and `homologation_requests` have a `discarded_at` column for soft delete.

| Action | What happens | Reversible? |
|---|---|---|
| "Archive" (coordinator/admin) | `discarded_at = Time.current`, hidden from lists | Yes — set `discarded_at = nil` |
| "Deactivate user" (super_admin) | `discarded_at = Time.current`, sessions destroyed | Yes |
| "GDPR full delete" (super_admin) | Hard delete record + files | No |

**Data retention policy (future `DataRetentionJob`):**
- Resolved/closed requests: files purged after 90 days (data already in AmoCRM)
- Soft-deleted records: hard-deleted after 90 days
- GDPR "right to erasure" request: immediate hard delete

**Model pattern:**
```ruby
# In User and HomologationRequest:
scope :kept, -> { where(discarded_at: nil) }
scope :discarded, -> { where.not(discarded_at: nil) }

def discard = update!(discarded_at: Time.current)
def undiscard = update!(discarded_at: nil)
def discarded? = discarded_at.present?
```

All queries default to `.kept` — discarded records are invisible unless explicitly requested.

## Encrypted Fields (GDPR)

| Model | Field |
|---|---|
| User | `phone`, `whatsapp`, `guardian_phone`, `guardian_whatsapp` |
| HomologationRequest | `identity_card`, `passport` |

## File Attachments (Active Storage, local disk, no S3)

| Model | Name | Type |
|---|---|---|
| HomologationRequest | `application` | has_one_attached |
| HomologationRequest | `originals` | has_many_attached |
| HomologationRequest | `documents` | has_many_attached |
| Message | `attachments` | has_many_attached |

## Seeds

```ruby
%w[super_admin coordinator teacher student].each { |r| Role.find_or_create_by!(name: r) }
```
